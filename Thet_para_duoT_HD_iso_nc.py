...
name_file = "Files/Para_750_1000_HD_iso_nc"
name_source = "Source"
name_exo = "HD209458"
#name_exo = "GJ1214b"
...
T_iso_array, P_surf, P_tau = np.array([750.,1000.]), 1.e+6, 1.e+3
...
save_adress = "/data1/caldas/Pytmosph3R/I_750_1000_HD_iso_nc/"
special = ''
if rank == 0 :
...
for beta_rad in beta_rad_array :
    ...
    if Profil == True :
        ...
        if TopPressure == True :
            T_max = np.amax(T_iso_array)
            alp = R_gp*T_max/(g0*M)*np.log(P_tau/P_surf)
            h_top_1 = round(-alp/(1+alp/Rp),-5)
            T_iso = np.amin(T_iso_array)
            g_iso = g0*(1/(1.+h_top_1/Rp))**2
            alp = R_gp*T_iso/(g_iso*M)*np.log(P_h/P_tau)
            h_top = round(-alp / (1 + alp / (Rp+h_top_1)), -5)
        ...
        T_iso, T_max = np.amin(T_iso_array), np.amax(T_iso_array)
                ...
                for i_n in range(n_layers+2) :
                    ...
                    if Composition == False :
                        if z < n_lim :
                            ...
                            data_convert[0,0,i_n,i_lat,i_long] = P_top*np.exp(-g0*(1/(1+z_maxi/Rp))**2*M/(R_gp*T_iso)*(z-z_maxi)/(1+(z-z_maxi)/(Rp+z_maxi)))
                        ...
                        if i_n == 0 :
                            ...
                            if i_n == 1 or i_n == n_layers+1 :
                                ...
                                delta = delta_z
                            data_convert[0,0,i_n,i_lat,i_long] = data_convert[0,0,i_n-1,i_lat,i_long]*np.exp(-g*data_convert[number-1,0,i_n,i_lat,i_long]\
                                                                    /(R_gp*data_convert[1,0,i_n,i_lat,i_long])*delta)
            if rank == 0 :
                bar.animate(i_lat+1)
