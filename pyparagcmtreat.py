...
def Boxes_interpolation(P,T,Q,Rp,g0,number,P_comp,T_comp,Q_comp,species,x_species,M_species,c_species,m_species,ratio,composition,\
                        Tracer=False,LogInterp=False,MassAtm=False,NoH2=False,TauREx=True) :
...
def cylindric_assymatrix_parameter(Rp,h,long_step,lat_step,r_step,theta_step,theta_number,x_step,z_level,phi_rot,\
                                   inc,phi_obli,reso_long,reso_lat,long_lat,rank,number_rank,Inclinaison=True,Obliquity=False,Middle=False) :
    ...
    q_lat_grid = np.ones((int(n_level_rank.size) ,theta_number , x_reso),dtype='int')*(-1)
    q_long_grid = np.ones((int(n_level_rank.size) ,theta_number , x_reso),dtype='int')*(-1)
    ...
    return q_lat_grid,q_long_grid,z_grid,n_level_rank
...
def dx_correspondance(data,path,x_step,delta_r,theta_number,Rp,g0,h,t,n_layers,reso_long,reso_lat,reso_alt,obs,n_lay_rank,\
    ...
    if Cylindric == True :
                    ...
                    for i_lo in range(q_init,q_end+1) :
                        ...
                        if theta == 0. or theta == np.pi :
                            if i_lo == 0 :
                                ...
                                n_long[i_lo], n_long[i_lo+1], n_long[i_lo+2] = q_long_ref_init, q_long_ref_end, q_long_ref_end
        ...
        order_grid = np.ones((3,n_lay_rank.size,theta_number,size_max),dtype=np.int)*(-1)
    ...
    pdx_grid = np.ones((n_lay_rank.size,theta_number,size_max),dtype=np.float64)*(-1)
    ...
    if Integral == True :
        ...
        if rank == 0 :
            bar = ProgressBar(n_lay_rank.size*theta_number,'Integration of pathes progression : ')

        for i_r in range(n_lay_rank.size) :
            if Middle == True :
                r = (n_lay_rank[i_r]+0.5)*delta_r
            else :
    ...
    return q_lat_grid, q_long_grid, q_z_grid, q_zh_grid, dx_grid_opt, pdx_grid, order_grid
