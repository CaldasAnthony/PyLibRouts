...
########################################################################################################################
    ...
    if Corr == True :
        ...
        if Profil == False :
            ...
            data_convert = np.load("%s%s/%s/%s_data_convert_%ix%ix%i_lo%.2f%s.npy"%(path,name_file,param_file,name_exo,reso_alt,\
                            reso_long,reso_lat,long_obs,stu_name))
        ...
        if Cylindre == True :
            ...
            np.save("%s%s/%s/dx_grid_opt_%i_%ix%ix%i_%i_%.2f_%.2f.npy"%(path,name_file,stitch_file,theta_number,reso_long,reso_lat,\
                    reso_alt,r_step,obs[0],obs[1]),dx_grid_opt)
            np.save("%s%s/%s/order_grid_%i_%ix%ix%i_%i_%.2f_%.2f.npy"%(path,name_file,stitch_file,theta_number,reso_long,reso_lat,\
                    reso_alt,r_step,obs[0],obs[1]),order_grid)

        if Integral == True :

            np.save("%s%s/%s/pdx_grid_%i_%ix%ix%i_%i_%.2f_%.2f.npy"%(path,name_file,stitch_file,theta_number,reso_long,reso_lat,\
...
########################################################################################################################
    ...
    if Matrix == True :
        ...
        if Cylindre == False or Corr == False :
            ...
            order_grid = np.load("%s%s/%s/order_grid_%i_%ix%ix%i_%i_%.2f_%.2f.npy"%(path,name_file,stitch_file,theta_number,reso_long,reso_lat,\
        ...
        if Tracer == True :
            np.save("%s%s/%s/%s_Q_%ix%ix%i_%i_%i_%.2f_%.2f.npy"%\
            ...
            if Cloudy == True :
                ...
                np.save("%s%s/%s/%s_compo_%ix%ix%i_%i_%i_%.2f_%.2f.npy"%\
                        (path,name_file,param_file,name_exo,reso_long,reso_lat,reso_alt,t_selec,r_step,obs[0],obs[1]),\
                            result[4])
        else :
            np.save("%s%s/%s/%s_Cn_%ix%ix%i_%i_%i_%.2f_%.2f.npy"%\
...
if Cylindric_transfert_3D == True :
    ...
    if Module == True :
        z_grid = np.load("%s%s/%s/q_z_grid_%i_%ix%ix%i_%i_%.2f_%.2f.npy"\
                %(path,name_file,stitch_file,theta_number,reso_long,reso_lat,reso_alt,r_step,obs[0],obs[1]))
    else :
        z_grid = np.array([])
...
########################################################################################################################
    ...
    if Kcorr == True :
        T_rmd = np.load("%s%s/%s/T_%ix%i_%s_%i_%ix%i_%i_rmd_%.2f_%.2f_%s.npy"\
                %(path,name_file,opac_file,reso_long,reso_lat,name_exo,t,dim_bande,dim_gauss-1,x_step,obs[0],obs[1],\
                  domain))
        P_rmd = np.load("%s%s/%s/P_%ix%i_%s_%i_%ix%i_%i_rmd_%.2f_%.2f_%s.npy"\
                %(path,name_file,opac_file,reso_long,reso_lat,name_exo,t,dim_bande,dim_gauss-1,x_step,obs[0],obs[1],\
                  domain))
        if Cl == True :
            gen_rmd = np.load("%s%s/%s/gen_%ix%i_%s_%i_%ix%i_%i_rmd_%.2f_%.2f_%s.npy"\
                %(path,name_file,opac_file,reso_long,reso_lat,name_exo,t,dim_bande,dim_gauss-1,x_step,obs[0],obs[1],\
                  domain))
        else :
            gen_rmd = np.array([])
        if Tracer == True :
            Q_rmd = np.load("%s%s/%s/Q_%ix%i_%s_%i_%ix%i_%i_rmd_%.2f_%.2f_%s.npy"\
                %(path,name_file,opac_file,reso_long,reso_lat,name_exo,t,dim_bande,dim_gauss-1,x_step,obs[0],obs[1],\
                  domain))
        else :
            Q_rmd = np.array([])
        rmind = np.load("%s%s/%s/rmind_%ix%i_%s_%i_%ix%i_%i_rmd_%.2f_%.2f_%s.npy"\
                %(path,name_file,opac_file,reso_long,reso_lat,name_exo,t,dim_bande,dim_gauss-1,x_step,obs[0],obs[1],\
                  domain))
    else :
        T_rmd = np.load("%s%s/%s/T_%ix%i_%s_%i_%i_%i_rmd_%.2f_%.2f_%s.npy"\
                %(path,name_file,opac_file,reso_long,reso_lat,name_exo,t,dim_bande,x_step,obs[0],obs[1],domain))
        P_rmd = np.load("%s%s/%s/P_%ix%i_%s_%i_%i_%i_rmd_%.2f_%.2f_%s.npy"\
                %(path,name_file,opac_file,reso_long,reso_lat,name_exo,t,dim_bande,x_step,obs[0],obs[1],domain))
        if Cl == True :
            gen_rmd = np.load("%s%s/%s/gen_%ix%i_%s_%i_%i_%i_rmd_%.2f_%.2f_%s.npy"\
                %(path,name_file,opac_file,reso_long,reso_lat,name_exo,t,dim_bande,x_step,obs[0],obs[1],domain))
        else :
            gen_rmd = np.array([])
        if Tracer == True :
            Q_rmd = np.load("%s%s/%s/Q_%ix%i_%s_%i_%i_%i_rmd_%.2f_%.2f_%s.npy"\
                %(path,name_file,opac_file,reso_long,reso_lat,name_exo,t,dim_bande,x_step,obs[0],obs[1],domain))
        else :
            Q_rmd = np.array([])
        rmind = np.load("%s%s/%s/rmind_%ix%i_%s_%i_%i_%i_rmd_%.2f_%.2f_%s.npy"\
                %(path,name_file,opac_file,reso_long,reso_lat,name_exo,t,dim_bande,x_step,obs[0],obs[1],domain))
...
########################################################################################################################
    ...
    for i_ca in range(wh_ca.size) :
        ...
        if os.path.isfile('%s.npy'%(save_name_3D_step)) != True or Push[i_ca] == True :
            ...
            if Molecular == True :
                if Kcorr == True :
                    k_rmd = np.load("%s%s/%s/k_corr_%ix%i_%s_%i_%ix%i_%i_rmd_%.2f_%.2f_%s.npy"\
                    %(path,name_file,opac_file,reso_long,reso_lat,name_exo,t,dim_bande,dim_gauss-1,x_step,obs[0],obs[1],domain))
                    gauss_val = np.load("%s%s/gauss_sample.npy"%(path,name_source))
                else :
                    if Optimal == True :
                        k_rmd = np.load("%s%s/%s/k_cross_opt_%ix%i_%s_%i_%i_%i_rmd_%.2f_%.2f_%s.npy"\
                        %(path,name_file,opac_file,reso_long,reso_lat,name_exo,t,dim_bande,x_step,obs[0],obs[1],domain))
                    else :
                        k_rmd = np.load("%s%s/%s/k_cross_%ix%i_%s_%i_%i_%i_rmd_%.2f_%.2f_%s.npy"\
                        %(path,name_file,opac_file,reso_long,reso_lat,name_exo,t,dim_bande,x_step,obs[0],obs[1],domain))
                    gauss_val = np.array([])
            else :
                if Kcorr == True :
            ...
            if Continuum == True :
                if Kcorr == True :
                    k_cont_rmd = np.load("%s%s/%s/k_cont_%ix%i_%s_%i_%ix%i_%i_rmd_%.2f_%.2f_%s.npy"\
                    %(path,name_file,opac_file,reso_long,reso_lat,name_exo,t,dim_bande,dim_gauss-1,x_step,obs[0],obs[1],domain))
                else :
                    k_cont_rmd = np.load("%s%s/%s/k_cont_%ix%i_%s_%i_%i_%i_rmd_%.2f_%.2f_%s.npy"\
                    %(path,name_file,opac_file,reso_long,reso_lat,name_exo,t,dim_bande,x_step,obs[0],obs[1],domain))
            else :
                k_cont_rmd = np.array([])
            ...
            if Scattering == True :
                if Kcorr == True :
                    k_sca_rmd = np.load("%s%s/%s/k_sca_%ix%i_%s_%i_%ix%i_%i_rmd_%.2f_%.2f_%s.npy"\
                    %(path,name_file,opac_file,reso_long,reso_lat,name_exo,t,dim_bande,dim_gauss-1,x_step,obs[0],obs[1],domain))
                else :
                    k_sca_rmd = np.load("%s%s/%s/k_sca_%ix%i_%s_%i_%i_%i_rmd_%.2f_%.2f_%s.npy"\
                    %(path,name_file,opac_file,reso_long,reso_lat,name_exo,t,dim_bande,x_step,obs[0],obs[1],domain))
            else :
                k_sca_rmd = np.array([])
            ...
            if Clouds == True :
                ...
                if Kcorr == True :
                    k_cloud_rmd = np.load("%s%s/%s/k_cloud_%ix%i_%s_%i_%ix%i_%i_rmd_%.2f_%.2f_%s_%s.npy" \
                    %(path,name_file,opac_file,reso_long,reso_lat,name_exo,t,dim_bande,dim_gauss-1,x_step,obs[0],obs[1],\
                      r_enn,domain))
                else :
                    k_cloud_rmd = np.load("%s%s/%s/k_cloud_%ix%i_%s_%i_%i_%i_rmd_%.2f_%.2f_%s_%s.npy" \
